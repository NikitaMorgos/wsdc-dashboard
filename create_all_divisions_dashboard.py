...
Генерирует все новые дашборды WSDC в папку charts/:
  ...
  charts/year_dynamics.html           — очки, заработанные в текущем году
...
def _page(title: str, body: str, active_link: str = "") -> str:
    nav_links = [
        ...
        ("../charts/year_dynamics.html", "📈 2026"),
        ("../index.html", "🏠 Главная"),
    ]
    nav_html = ""
...
# ──────────────────────────────────────────────────────────────────────────────
# Дашборд 4: Очки за текущий год
# ──────────────────────────────────────────────────────────────────────────────
def build_year_points_rows(placements: List[Dict], year: int) -> List[Dict]:
    """Сумма очков по (wsdc_id, role, division) только за ивенты указанного года."""
    grouped: Dict[Tuple[str, str, str], Dict] = {}
    for p in placements:
        dt = p.get("event_dt")
        if not dt or dt.year != year:
            continue
        wid = (p.get("wsdc_id") or "").strip()
        if not wid:
            continue
        role = (p.get("role") or "").strip()
        div = (p.get("division") or "").strip()
        if not role or not div:
            continue
        key = (wid, role, div)
        ...
        g = grouped[key]
        g["points"] += p.get("points") or 0
        ev = p.get("event") or ""
        if ev:
            g["events"].add(ev)
        nm = (p.get("name") or "").strip()
        if nm and (not g["name"] or len(nm) > len(g["name"])):
            g["name"] = nm
...
def build_year_dynamics_dashboard(placements: List[Dict], year: int = CURRENT_YEAR) -> str:
    year_rows = build_year_points_rows(placements, year)
    ...
    for div in DIVISION_ORDER:
        color = DIVISION_COLORS.get(div, "#4361ee")
        js_data[div] = {"color": color, "Leader": [], "Follower": []}
        for role in ("Leader", "Follower"):
            for i, r in enumerate(data[div][role]):
                js_data[div][role].append({
                    "rank": i + 1,
    ...
    for div in DIVISION_ORDER:
        ...
        color = DIVISION_COLORS.get(div, "#4361ee")
        tab_btns += (
            f'<button class="tab-btn" data-div="{div}" onclick="switchDiv(this)" '
            f'style="border-color:{color}30">{div}</button>\n'
        )

    body = f"""
<a href="../index.html" class="back-link">← Главная</a>
<div class="page-header">
  <h1>📈 Динамика за {year} год</h1>
  <p>Сколько очков заработано на ивентах {year} года · {today_str()}</p>
  <p style="max-width:720px;margin:10px auto 0;color:var(--muted);font-size:.9rem;line-height:1.45">
    Сумма очков по каждому ивенту WSDC с датой в {year} году. Один танцор может быть в нескольких дивизионах
    (если выступал в разных конкурсах). Рейтинг внутри дивизиона и роли.
  </p>
  <div class="meta">Источник: points.worldsdc.com · danceConvention</div>
</div>
...
function renderAll() {{
  ...
  document.getElementById('statsRow').innerHTML = `
    <div class="card stat-box" style="flex:1;min-width:100px">
      <div class="stat-num" style="color:var(--blue)">${{allRows.length}}</div>
      <div class="stat-lbl">танцоров с очками</div>
    </div>
    <div class="card stat-box" style="flex:1;min-width:100px">
      <div class="stat-num">${{totalPts}}</div>
      <div class="stat-lbl">сумма pts за ${{YEAR}}</div>
    </div>
    <div class="card stat-box" style="flex:1;min-width:100px">
      <div class="stat-num">${{maxPts}}</div>
      <div class="stat-lbl">максимум pts</div>
  ...
  yearChart = new Chart(ctx, {{
    ...
    options: {{
      ...
      plugins: {{ legend: {{ display: false }},
        tooltip: {{ callbacks: {{
          label: c => ' ' + c.parsed.y + ' pts за ' + YEAR,
        }} }} }},
      scales: {{
        x: {{ ticks: {{ color: '#8b949e', maxRotation: 45, font: {{size: 10}} }},
               grid: {{ color: '#21262d' }} }},
  ...
  html += '</tbody></table></div>';
  if (!rows.length) html = '<p style="color:var(--muted);padding:20px">Нет очков за этот год в выбранном дивизионе</p>';
  document.getElementById('tableWrap').innerHTML = html;
}}
</script>
"""
    return _page(f"Динамика за {year} год", body, "../charts/year_dynamics.html")
...
def main() -> int:
    ...
    pages = [
        ...
        ("year_dynamics.html",       build_year_dynamics_dashboard(placements)),
    ]
